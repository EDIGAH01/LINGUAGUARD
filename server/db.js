...
function ensureSeedAdmin() {
  ...
  if (!process.env.SEED_ADMIN_PASSWORD) {
    // Never put the actual secret in stdout — process logs routinely end up
    // somewhere with a wider audience than "whoever ran npm run server"
    // (Docker logs, a systemd journal, a hosting platform's log viewer, a
    // terminal someone is screen-sharing). A local file with owner-only
    // permissions is at least no more exposed than .env already is, and it
    // doesn't linger in every place stdout gets copied to.
...
module.exports = { load, save, ensureSeedAdmin, getKv, setKv };
